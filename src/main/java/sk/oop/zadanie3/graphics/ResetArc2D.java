...
public class ResetArc2D extends Arc2D implements Drawable{ //trieda pouzita na vykreslenie Reset hrany
    private Arc arc;

    public ResetArc2D(Arc arc, int x1, int y1, int x2, int y2){
    ...
    public Polygon createArrow(){// vytvorenie pozadovanej sipky ktora sa pouzije na vykreslenie
        ...
        arrowHead.addPoint(-1, -10);
        arrowHead.addPoint(-5, -20);
        arrowHead.addPoint(5, -20);
        arrowHead.addPoint(1, -10);
        arrowHead.addPoint(5, -10);
        return arrowHead;
